...
| Régénérer les données des pages par ville (`cities-seo.ts`) | `npm run build:seo-cities` | racine |
...
À la racine, ce qui ne relève d'aucune des trois : `config.js`, `protocol.js` (primitives de fil `clamp`/`ack`/`genId`, partagées par les deux transports), `security.js`, `metrics.js`, `audience.js` (collecte d'audience **Express-aware** — middleware et échantillonneur — au-dessus de `domain/analytics.js`, comme `security.js` est Express-aware), `admin.js` (namespace `/admin`) et `room-actions.js` (actions de cycle de vie **io-aware**, partagées par `handlers/` et `admin.js` — d'où sa position hors de `domain/`).

**Deux modules de chiffres, deux questions distinctes.** `metrics.js` répond à « où en est-on **maintenant** ? » (instantané poussé toutes les 5 s tant qu'un opérateur regarde) ; `domain/analytics.js` répond à « d'où vient le monde et **comment ça évolue** ? » (compteurs journaliers à TTL, lus **à la demande** — les balayer sur le tick coûterait un parcours permanent pour un chiffre qui ne bouge qu'à l'échelle de la journée).
...
**Frontend `frontend/src/`** — ESM, Vue 3.5 + TypeScript + Vite 7 + Tailwind 4, état dans Pinia (`store/useStore.ts`), temps réel via `socket.io-client`. Le chiffrement E2E client est dans `lib/crypto.ts` (libsodium).
...
- Backend = CommonJS (`require`, `module.exports`, `'use strict'`). Frontend = ESM/SFC Vue (`<script setup lang="ts">`). Ne pas mélanger.
...
- **Les statistiques ne pistent personne** (`domain/analytics.js`) : mesure **côté serveur**, sans script, sans cookie, sans identifiant de visiteur — c'est ce qui laisse vraies, mot pour mot, les promesses publiques de `Confidentialite.vue` (« aucun traceur publicitaire ni analytique tiers ») et de `ChatAnonyme.vue` (« la page ne charge rien qui ne vienne du site »). Trois choix portent cette promesse et ne se défont pas séparément :
  - le comptage des visites passe par un **HyperLogLog** (`PFADD`) alimenté par le hash d'IP **à sel rotatif** de `security.hashIp` : Redis ne garde qu'une esquisse dont on ne peut pas ressortir les membres. La contrepartie est dans le nom — ce sont des **« visites »**, pas des « visiteurs uniques » : le sel tournant toutes les 5 minutes, une longue session compte plusieurs fois. Le **sel quotidien** des outils du marché donnerait le vrai unique en étendant la corrélation d'IP de 5 minutes à 24 heures : c'est un arbitrage RG-08, à soumettre au porteur du projet, pas à trancher en passant ;
  - seules les **routes déclarées** sont comptées (`pages.js`, `city-pages.js`, résolues par `audience.js`) : aucune chaîne venue de l'extérieur n'atteint Redis, donc aucune cardinalité non bornée ;
  - tout porte un **TTL** (`analytics.retentionDays`, 30 j par défaut). RG-01 vaut aussi pour les statistiques : elles vieillissent et disparaissent seules.
...
- Les URLs indexables sont **déclarées**, jamais devinées : les 11 pages écrites à la main dans `server/pages.js`, les 67 pages par ville dans `server/city-pages.js`. Quatre consommateurs doivent rester d'accord (routes serveur, `scripts/prerender-routes.js`, `frontend/src/lib/router.ts`, sitemap) — `test/pages.test.js` et `test/city-pages.test.js` échouent sinon. Tout autre chemin renvoie une vraie 404 : ne pas rétablir un fallback `app.get('*')` vers `index.html`, qui recréerait des « soft 404 » aux yeux des moteurs.
- Les **pages par ville** (`/tchat/nancy`) sont dérivées de la base géographique, pas rédigées : `server/city-pages.js` porte la sélection (France ≥ 100 000 habitants + une liste nommée pour BE/CH/LU/MC, où les populations de la base sont inégales) et calcule les communes réellement à portée. Ce calcul est ce qui rend ces pages légitimes plutôt que des *doorway pages* : sans donnée propre à la ville, ne pas ajouter de page. `frontend/src/lib/cities-seo.ts` est **généré** (`npm run build:seo-cities`) et versionné, comme `cities.json` — un oubli de régénération casse `test/city-pages.test.js`, pas la production.
- `sitemap.xml` n'existe plus dans `frontend/public/` : il est **généré au build** par `scripts/prerender-routes.js` depuis les deux déclarations de pages. Ne pas en recréer un à la main, il divergerait au premier changement de sélection.
...
- Les compteurs `stats:*` **survivent à la remise à zéro** opérateur (cf. `domain/purge.js`) : ce n'est pas de la conversation mais l'historique du site, et le geste sert à repartir d'une plateforme vide, pas à effacer le mois écoulé. Ils n'ont pas non plus besoin d'être purgés — leur TTL s'en charge.
- Une page de la SPA atteinte par le **routage client** ne repasse pas par le serveur : `stats:pv` compte donc des **chargements de document**, c'est-à-dire les pages d'**entrée**. L'interface le dit ainsi ; ne pas la laisser dériver vers « pages vues », qui promettrait une navigation interne qu'on ne mesure pas.
