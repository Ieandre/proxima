...
import type { RoomCardTarget } from '../rooms/card';
import TopBar from '../layout/TopBar.vue';
import { Icon } from '../ui';

const st = useStore();
const { active } = storeToRefs(st);
...
const pending = ref<(RoomCardTarget & { password?: string }) | null>(null);
...
onMounted(() => {
  ...
  if (i) {
    claimInvite(i).then((res) => {
      if (!res.ok) st.showToast(res.error || 'Cette invitation a expiré.', 'warn');
    });
    return;
  }
  if (!r) return;
  ...
  if (k) {
    joinRoom({ roomId: r, invite: k }).then((res) => {
      if (!res.ok) st.showToast(res.error || 'Invitation invalide.', 'warn');
    });
    return;
  }
  peekRoom(r).then((res) => {
    if (!res.ok) {
      st.showToast(res.error || 'Salon introuvable ou fermé.', 'warn');
      return;
    }
    ...
    if (res.keyMode === 'password' && res.salt) {
      ...
      joinRoom({ roomId: r }).then((j) => {
        if (!j.ok) st.showToast(j.error || 'Impossible de rejoindre.', 'warn');
      });
    }
  });
});
  ...
  <div class="flex h-full flex-col">
    ...
    <TopBar column="app" :sticky="false">
      ...
      >
        ...
        <span class="hidden lg:inline">Comment ça marche</span>
      </a>
      <button class="btn btn-ghost px-3" @click="quit" title="Quitter et tout détruire">
    ...
    <div class="app-shell grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[clamp(280px,26vw,340px)_minmax(0,1fr)]">
      <div :class="`min-h-0 border-r border-line ${active ? 'hidden md:block' : 'block'}`">
        <Sidebar />
      </div>
      <div :class="`min-h-0 ${active ? 'block' : 'hidden md:block'}`">
        <Conversation />
      </div>
    </div>
